template<class T, class T2> inline void chkmax(T &x, const T2 &y) {
  if(x < y) x = y;
}
template<class T, class T2> inline void chkmin(T &x, const T2 &y) {
  if(x > y) x = y;
}
const double PI = acos(-1);
const int MAXN = (1 << 19);
const int maxn = (int)2e5 + 42 + 17;
...
vector<int> mult(vector<int> a, vector<int> b) {
  ...
  int lg = 0;
  while((1 << lg) < (int)(a.size() + b.size())) ++lg;
